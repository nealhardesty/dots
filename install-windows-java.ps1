# Run as administrator: powershell -ExecutionPolicy Bypass -File install-windows-java.ps1
...
Write-Host "Installing OpenJDK 21 via winget..." -ForegroundColor Cyan
...
$jdkDir = Get-ChildItem "$jdkRoot" -Directory | Where-Object { $_.Name -like "jdk-21*" } | Sort-Object LastWriteTime -Descending | Select-Object -First 1

if (-not $jdkDir) {
    Write-Host "ERROR: Failed to locate JDK 21 install directory." -ForegroundColor Red
    exit 1
}
